// src/features/cms/Blocks/BlocksSlice.jsx
import { createSlice } from "@reduxjs/toolkit";
import {
  fetchHeaderBlocks,
  fetchNewsLetterBlocks,
  fetchBannerBlocks,
  fetchFooterBlocks,
  fetchMarketingCampaigns,
  fetchSEOSettingsList,
  fetchAnalyticsSettingsList,
  fetchAmbassadorBlocks, 
} from "./BlocksAction";
...
const initialState = {
  ...
  ambassadorBlocks: [], // Added state for Ambassador blocks
...
const blocksSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // Ambassador Blocks
      .addCase(fetchAmbassadorBlocks.pending, (state) => {
        state.ambassadorLoading = true;
        state.ambassadorError = null;
      })
      .addCase(fetchAmbassadorBlocks.fulfilled, (state, action) => {
        state.ambassadorLoading = false;
        state.ambassadorBlocks = action.payload?.results || [];
      })
      .addCase(fetchAmbassadorBlocks.rejected, (state, action) => {
        state.ambassadorLoading = false;
        state.ambassadorError = action.payload;
      })
